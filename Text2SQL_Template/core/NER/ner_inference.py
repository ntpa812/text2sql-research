"""
NER Inference Module - Service-ready (local copy for Text2SQL_Template)
...
# Resolve model path relative to Text2SQL_Template project root
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
_DEFAULT_NER_MODEL_PATH = os.path.join(
    _PROJECT_ROOT, os.pardir, "6804_DDQ",
    "data_models", "ner_model", "NER_New_Model_14_entities_running",
)

# Use logging instead of mPs_ner.mlog
print = logger.info
...
class NERInference:
    ...
    def __init__(self, model_path: Optional[str] = None, device: Optional[torch.device] = None):
        self._model_path = model_path or _DEFAULT_NER_MODEL_PATH
...
# ============================================================
# LAZY LOADING: Model chỉ load khi gọi lần đầu
# ============================================================

ner_engine: Optional[NERInference] = None


def _get_engine() -> NERInference:
    global ner_engine
...
def ner_predict(query: str, max_len: int = 200) -> List[Dict[str, str]]:
    ...
    engine = _get_engine()
    print(f"\n[NER] Câu gốc: {query}")
    result = engine.predict(query, max_len)
...
def ner_predict_raw(query: str, max_len: int = 200) -> List[Dict[str, str]]:
    ...
    Returns:
        List các dict {entity_type: entity_value}
    """
    return _get_engine().predict(query, max_len)

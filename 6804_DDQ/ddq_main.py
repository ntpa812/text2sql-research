...
from ddq_data_pool import get_data_from_lake
...
from config import mPs_VietnameseNormalizer, mPs_ner, mPs_PreProcessing
mPs_intent = mPs_VietnameseNormalizer
...
_server_port = int(os.environ.get("PORT", 6804))
...
def get_value_from_Microservices(URL="http://192.168.3.7:6803/api/fix_Vietnamese_Errors", inputJson={}):
    """
     curl -X 'POST' \
        'http://192.168.3.7:6803/api/fix_Vietnamese_Errors' \
        -H 'accept: application/json' \
        -H 'Content-Type: application/json' \
        -d '{ "query": "tai khoan ngan hang", "chunk_size": 24, "model_type": "teacher" }'

    output:
        {
        "status": 1,
        "original_text": "tai khoan ngan hang",
        "corrected_text": "tài khoản ngân hàng",
        "confidence": 0.95,
    ...
    try:
        ...
        log_func("get_value_from_Microservices", f"Error calling microservice: {e}", debug_level=0)
        return {
            "status": -1,
...
class outputIntentRecognition(BaseModel):
    ...
    score: float=0.0
    question: str = ""
    metadata: Optional[Union[str, Dict, List]] = None
    tracking: Optional[dict]={}
...
# ============================================================================
# Fix Vietnamese Errors (OCR errors, missing diacritics, etc.)
# ============================================================================
from fix_Vietnamese_errors.vietnamese_text_corrector import VietnameseTextCorrector

# Initialize corrector singleton (loads model once at startup)
_vietnamese_corrector: Optional[VietnameseTextCorrector] = None

def get_vietnamese_corrector() -> VietnameseTextCorrector:
    """Lazy-load Vietnamese text corrector model."""
    global _vietnamese_corrector
    if _vietnamese_corrector is None:
        mPs_intent.mlog("Loading Vietnamese Text Corrector model...")
        _vietnamese_corrector = VietnameseTextCorrector(model_type="teacher")
        mPs_intent.mlog("Vietnamese Text Corrector model loaded successfully")
    return _vietnamese_corrector


class FixErrorInput(BaseModel):
    """Input model for Vietnamese text correction API."""
    query: str
    chunk_size: int = 24  # Words per chunk for long text


class FixErrorOutput(BaseModel):
    """Output model for Vietnamese text correction API."""
    status: int = 1
    original_text: str
    corrected_text: str
    message: str = "success"
...
# @app.post("/fix_Vietnamese_Errors", tags=["Grammarly"], response_model=FixErrorOutput)
# def fix_Vietnamese_Errors(item: FixErrorInput) -> FixErrorOutput:
#     """
#     Correct Vietnamese text errors (OCR errors, missing diacritics, typos, etc.)
    
#     Uses ProtonX Legal Text Correction model to fix:
#     - Missing Vietnamese diacritics (e.g., "cong hoa" -> "cộng hòa")
#     - OCR errors (e.g., "Đièu" -> "Điều")
...
#         ```
#         {
#             "query": "can cu bo luat lao dong 2019 va cac van ban huong dan thuc hien"
#         }
#         ```
...
#         {
#             "status": 1,
#             "original_text": "can cu bo luat lao dong 2019 va cac van ban huong dan thuc hien",
#             "corrected_text": "căn cứ bộ luật lao động 2019 và các văn bản hướng dẫn thực hiện",
#             "message": "success"
#         }
#         ```
    
#     Args:
#         item: FixErrorInput containing the text to correct
        
#     Returns:
#         FixErrorOutput with original and corrected text
#     """
#     try:
#         query = item.query.strip()
#         if not query:
#             return FixErrorOutput(
#                 status=0,
#                 original_text="",
#                 corrected_text="",
...
#         corrector = get_vietnamese_corrector()
        
#         # Use long text processing if text is longer than chunk_size words
#         word_count = len(query.split())
#         if word_count > item.chunk_size:
#             corrected = corrector.correct_long_text(query, chunk_size=item.chunk_size)
#         else:
#             corrected = corrector.correct(query)
        
#         mPs_intent.mlog(f"[fix_Vietnamese_Errors] '{query[:50]}...' -> '{corrected[:50]}...'")
...
#     except Exception as e:
#         mPs_intent.mlog(f"[fix_Vietnamese_Errors] Error: {str(e)}")
#         return FixErrorOutput(
#             status=-1,
...
def _process_batch_search(file_content: bytes, top_k: int, original_filename: str, client_ip: str) -> tuple:
    ...
    if 'API' in df.columns:
        df = df.rename(columns={'API': 'Documents'})
    colQuery = 'query' if 'query' in df.columns else 'question'
    api_col = next((c for c in df.columns if c.lower().strip() == 'documents'), None)
    ...
    for i, row in df.iterrows():
        ...
        for result in search_results:
            ...
            if api_col:
                ...
                result_row['expected_Documents'] = expected_api
    ...
    if api_col:
        cols_order.extend(['expected_Documents', 'is_match'])
...
async def search_batch_stream(request: Request, file: UploadFile = File(...), top_k: int = config.DEFAULT_TOP_K):
    ...
    async def generate_events():
        ...
        try:
            ...
            if 'API' in df.columns:
                df = df.rename(columns={'API': 'Documents'})
            has_documents_column = 'Documents' in df.columns
            ...
            for i, row in df.iterrows():
                ...
                for result in search_results:
                    ...
                    if has_documents_column:
                        expected_documents = str(row.get('Documents', '')).strip() if pd.notna(row.get('Documents')) else ''
                        result_row['expected_Documents'] = expected_documents
                        if expected_documents:
                            doc_lower = str(result['document']).lower()
                            documents_lower = expected_documents.lower()
                            result_row['is_match'] = 1 if (documents_lower in doc_lower or doc_lower in documents_lower) else 0
...
@app.post("/DDQ/intent_recognition", tags=["DDQ"])
async def intent_recognition(item: inputIntentRecognition):
    ...
    question_normalized=item.question
    if tracking:
        if 'normalized' not in tracking:
            try:
                # question = normalize_text(item.question) or "" 
                # question_normalized = vietnamese_normalizer.normalize_for_intent(question)    
                # mFixErrorOutput = fix_Vietnamese_Errors(FixErrorInput(query=question_normalized)) 
                # question_normalized=mFixErrorOutput.corrected_text 
                url=mPs_PreProcessing.fix_Vietnamese_Errors_URL 
                inJson={ "query": item.question, "chunk_size": 24, "model_type": "teacher" } 
                oJson = get_value_from_Microservices(url,inJson)
                question_normalized = oJson.corrected_text # type: ignore
                tracking['normalized'] = True  
            except Exception as e:
                print("normalized:", e)
    ...
    #     })
    # print(KQ)
    groupIntent = KQ[0]['document'].split("|")[0].strip()
    intent=KQ[0]['document'].split("|")[-1].strip()
    mPs_intent.mlog(f"======\ngroupIntent:[{groupIntent}], intent: [{intent}], question:[{question_normalized}], score:[{float(KQ[0]['score'])}]")
    tracking['SQL'] = True
    return outputIntentRecognition(status=1, 
                                   groupIntent=groupIntent, 
                                   intent=intent, 
                                   options=[ kq1 for kq1 in KQ ], 
                                   score=float(KQ[0]['score']), 
                                   question=question_normalized,
                                   metadata=KQ[0]['metadata'], 
                                   tracking=tracking
                                   )     
...
from core.NER.ner_inference import ner_predict_raw
    
@app.post("/DDQ/NER", tags=["DDQ"])
async def get_entity_NER(item: inputNER):
    """
    ...
    try:
        ...
        # Chuẩn hóa Unicode NFC cho question
        question = normalize_text(item.question) or ""
        results = ner_predict_raw (question)
        mPs_ner._log(f"[NER] [QUESTION]: {question}")
        mPs_ner._log(f"[NER] [RESULT]: {results}")

        return outputNER(status=1, results = results) 

    except Exception as e:
        print(str(e))
        return outputNER(status=-1, results=[])
    

class outputDDQ(BaseModel):  
    question:str=""
    group_intent:str=""
    intent:str=""
    intent_score:float=0.0
    ner_objects:Optional[Union[str, Dict, List]] = None
    SQL:str=""
    SQLready:str=""
    llm_context:str=""
...
#     score: float=0.0
#     question: str = ""    
@app.post("/DDQ/full_pipeline", tags=["DDQ"])
async def full_DDQ_pipeline(inpItem: inputIntentRecognition):
    """
    Full DDQ pipeline combining Intent Recognition and Named Entity Recognition.

    This endpoint processes a user question through the complete DDQ (Dynamic Data Query) pipeline:
    1. Intent Recognition: Identifies the user's intent and maps it to appropriate API endpoints
    2. Named Entity Recognition: Extracts relevant entities (account numbers, dates, etc.) from the question
    3. Combines results into a structured response for downstream processing

    Args:
        item (inputIntentRecognition): Input containing the user's question

    Returns:
        outputDDQ: Structured response containing:
            - question: Normalized question text
            - group_intent: SQL/API group/category
            - intent: Specific SQL/API endpoint
            - intent_score: Confidence score for intent recognition
            - ner_objects: Extracted named entities with mapped keys
            - SQL: Metadata/SQL information from intent recognition
    ...
    Example:
        ...
        ```
        {
            "question": "Cho tôi xem lịch sử giao dịch của tài khoản 020063535533"
            "account_id":
        }
        ```
        ...
        {
            "question": "cho tôi xem lịch sử giao dịch của tài khoản 020063535533",
            "group_intent": "Transaction",
            "intent": "getTransactionHistory",
            "intent_score": 0.95,
            "ner_objects": {
                "account_number": "020063535533"
            },
            "SQL": "{\"table\": \"transactions\", \"filters\": {\"account_id\": \"020063535533\"}}"
        }
        ```
    """
    # cách câu SQL hỗ trợ:
    ...
    ddq_intent = await intent_recognition(item=inpItem)

    # Step 2: Named Entity Recognition
    question_normalized = ddq_intent.question
    input_ner = inputNER(question=question_normalized)
    ddq_ner = await get_entity_NER(item=input_ner)

    # Step 3: Process NER results and map entity keys
    ner_object = {}
    if inpItem.user_id:
        ner_object['user_id']=inpItem.user_id
    if inpItem.user_name:
        ner_object['user_name']=inpItem.user_name
    if inpItem.account_number:
        ner_object['account_number']=inpItem.account_number
        
    
    for result_dict in ddq_ner.results:
        for key, val in result_dict.items():
            if key in mPs_ner.Ner_Key_Mapping.keys():
                new_key = mPs_ner.Ner_Key_Mapping[key]
                ner_object[new_key] = val
            else:
                ner_object[key] = val
    SQL=ddq_intent.metadata 
    # Generate SQLready by replacing placeholders in SQL template with NER values
    SQLready = ""
    if isinstance(SQL, dict) and 'raw_text' in SQL:
        sql_template = SQL['raw_text']
        sql_template = sql_template.replace("\\n", " ")
        SQL['raw_text']=sql_template
        ...
        # Replace placeholders like {account_number} with actual values from ner_object
        for placeholder, value in ner_object.items():
            placeholder_pattern = f"{{{placeholder}}}"
            sql_template = sql_template.replace(placeholder_pattern, str(value))
        
        # Thêm giới hạn: user_id và user_name nếu nó tồn tại trong ner_object rồi đồng bộ dữ liệu một thể
        user_constraints = []
        if 'user_id' in ner_object and ner_object['user_id']:
            user_constraints.append(f"user_id = '{ner_object['user_id']}'")
        if 'user_name' in ner_object and ner_object['user_name']:
            user_constraints.append(f"user_name = '{ner_object['user_name']}'")
        
        
        if user_constraints:
            constraint_clause = " AND ".join(user_constraints)
            sql_upper = sql_template.upper()
            
            if 'WHERE' in sql_upper:
                # Tìm vị trí của các keyword sau WHERE để chèn điều kiện
                end_keywords = ['ORDER BY', 'GROUP BY', 'LIMIT', 'HAVING', ';']
                insert_pos = len(sql_template)
                for kw in end_keywords:
                    pos = sql_upper.find(kw.upper())
                    if pos != -1 and pos < insert_pos:
                        insert_pos = pos
                # Chèn AND clause trước keyword
                sql_template = sql_template[:insert_pos].rstrip() + ' AND ' + constraint_clause + ' ' + sql_template[insert_pos:]
            else:
                # Không có WHERE clause, thêm mới
                end_keywords = ['ORDER BY', 'GROUP BY', 'LIMIT', 'HAVING', ';']
                insert_pos = len(sql_template)
                for kw in end_keywords:
                    pos = sql_upper.find(kw.upper())
                    if pos != -1 and pos < insert_pos:
                        insert_pos = pos
                sql_template = sql_template[:insert_pos].rstrip() + ' WHERE ' + constraint_clause + ' ' + sql_template[insert_pos:]
        ...
        SQLready=' '.join(sql_template.split())
    
    
    llm_context=get_data_from_lake(SQLready, question_normalized)
    
    # Step 4: Build final response    
    result = outputDDQ(
        question=question_normalized,
        group_intent=ddq_intent.groupIntent,
        intent=ddq_intent.intent,
        intent_score=ddq_intent.score,
        ner_objects=ner_object,
        SQL=str(ddq_intent.metadata),
        SQLready=SQLready,
        llm_context=llm_context
    )
    # {
    # "question": "cho tôi xem lịch sử giao dịch của tài khoản 020063535533",
    # "group_intent": "Tra cứu lịch sử giao dịch theo số tài khoản",
    # "intent": "Tra cứu lịch sử giao dịch theo số tài khoản",
    # "intent_score": 0.8628,
    # "ner_objects": {
    #     "account_number": "020063535533"
    # },
    # "SQL": "{'raw_text': \"SELECT trans_id, trans_time, trans_type, from_account_no, to_account_no, \\n       amount_transfer, amount_currency, trans_status, trans_desc\\nFROM transaction \\nWHERE from_account_no = '{account_number}' \\n   OR to_account_no = '{account_number}'\\nORDER BY trans_time DESC;\"}",
    # "SQLready": "SQLready"
    # }
    return result
